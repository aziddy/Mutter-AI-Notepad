/**
 * Hybrid Diarization Service
 *
 * Combines:
 * - whisper.cpp for transcription (Metal GPU accelerated on Mac)
 * - pyannote for speaker diarization (CPU)
 *
 * This provides faster performance on Mac compared to WhisperX,
 * which cannot use GPU acceleration on Apple Silicon.
 */
...
class HybridDiarizationService {
  /**
   * Create a new HybridDiarizationService instance.
  ...
  async checkEnvironment() {
    ...
    return {
      ready,
      message: ready ? 'Environment ready (hybrid mode)' : messages.join('; '),
      details: checks,
    };
  }
  ...
  async runPyannote(audioPath, onProgress) {
    return new Promise((resolve, reject) => {
      ...
      console.log(`[Hybrid] Running pyannote: ${python}`);
  ...
  async diarize(audioPath, options = {}) {
    ...
    console.log('[Hybrid] Starting hybrid diarization pipeline');
    console.log('[Hybrid] Step 1: Running whisper.cpp (Metal GPU)...');

    // Step 1: Run whisper.cpp for transcription
    let transcription;
    try {
      transcription = await this.transcriptionService.transcribe(audioPath);
      console.log('[Hybrid] Whisper transcription complete');
    } catch (error) {
      return {
        success: false,
    ...
    console.log('[Hybrid] Step 2: Running pyannote (CPU)...');

    let pyannoteResult;
    try {
      pyannoteResult = await this.runPyannote(audioPath, options.onProgress);
      console.log('[Hybrid] Pyannote diarization complete');
    } catch (error) {
      return {
        success: false,
    ...
    console.log('[Hybrid] Step 3: Aligning speakers with transcription...');
    ...
    console.log(`[Hybrid] Pipeline complete in ${duration.toFixed(1)}s`);
...
module.exports = { HybridDiarizationService };
